...
(defun ghcid-stack-cmd (target)
  (format "stack ghci %s \
--test --bench \
--ghci-options=-fno-code --ghci-options=-fno-break-on-exception --ghci-options=-fno-break-on-error \
--ghci-options=-v1 --ghci-options=-ferror-spans --ghci-options=-j" target))
...
;; TODO Create an ivy auto-completion list from stack targets
(defun ghcid-set-target ()
  "Interactively set the target of ghcid"
  (interactive)
  (setq ghcid-target (read-from-minibuffer "ghcid target: " ghcid-target)))


(defun ghcid-change-target ()
  "Set the target and restart ghcid"
  (interactive)
  (ghcid-set-target)
  (ghcid-restart))


(evil-set-initial-state 'ghcid-mode 'normal)
